...
def _migrate_questions_schema(conn):
    ...
    if "passage" not in cols:
        conn.execute("ALTER TABLE questions ADD COLUMN passage TEXT NOT NULL DEFAULT ''")
    if "chart_data" not in cols:
        conn.execute("ALTER TABLE questions ADD COLUMN chart_data TEXT NOT NULL DEFAULT ''")
