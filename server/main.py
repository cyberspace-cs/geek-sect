...
# ---- 加载本地 .env（零依赖，避免引入 python-dotenv）----
# 在 import agent.llm / 读取 LLM_PROVIDER 等环境变量之前执行，确保密钥生效。
...
from models import (
    ...
    UserOut, UserRegister, WxLoginIn, WrongBookIn, WrongBookOut,
...
def derive_subcat(cat: str, topic: str) -> str:
    ...
    if cat == "考公" and topic == "政治":
        return "行测"
    if cat == "考公" and (topic or "").startswith("资料分析"):
        return "资料分析"
    if cat == "大厂" and ("面试题" in (topic or "") or (topic or "").startswith("大厂面试")):
        return "大厂面试·问答题"
    return topic
...
def page_questions(
    ...
    rows = conn.execute(
        "SELECT id, cat, src, type, topic, subcat, difficulty, stem, src_type, year, passage, chart_data FROM questions WHERE 1=1"
        + where
...
def daily_question():
    ...
    conn = get_db()
    rows = conn.execute(
        "SELECT id, cat, src, type, stem, opts, answer, explain, topic, difficulty, passage, chart_data "
...
def check_quiz(data: QuizCheckIn):
    ...
    for item in data.items:
        ...
        q = dict(row)
        if q["type"] == "问答题":
            # 问答题不自动判分，仅返回参考回答供自我复盘（考察系统学习与独立思考）
            results.append({
                "id": q["id"],
                "correct": None,
                "reference": q["answer"],
                ...
                "chart_data": q.get("chart_data", "") or "",
                "type": q["type"],
            })
            continue
...
@app.post("/api/wxlogin")
def wx_login(data: WxLoginIn):
    """微信小程序一键登录：用 wx.login() 换取的 code 调 code2session 拿 openid，
    自动注册/复用本地账号（username=wx_<openid>），返回与 /api/login 一致的 user_id。"""
    ...
    if not appid or not secret:
        raise HTTPException(500, "服务端未配置微信登录（缺少 WECHAT_APPID/WECHAT_APPSECRET）")
    try:
        resp = httpx.get(
        ...
        print("[wxlogin] 调用微信接口失败:", e)
        raise HTTPException(502, "微信登录接口调用失败")
    if "openid" not in info:
        raise HTTPException(401, info.get("errmsg", "微信登录失败"))
    openid = info["openid"]
    uname = "wx_" + openid
    conn = get_db()
    row = conn.execute("SELECT * FROM users WHERE username=?", (uname,)).fetchone()
    if not row:
        # 密码用 openid+secret 派生，仅占位（微信登录不走密码），保证 users 表约束成立
        h = hash_password(openid + secret)
        cur = conn.execute("INSERT INTO users (username, password_hash) VALUES (?,?)", (uname, h))
        conn.commit()
        row = conn.execute("SELECT * FROM users WHERE id=?", (cur.lastrowid,)).fetchone()
    conn.close()
    return {"id": row["id"], "user_id": row["id"], "username": row["username"],
            "created_at": row["created_at"] or "", "wx": True}
...
def get_wrong_book(user_id: int):
    ...
    rows = conn.execute("""
        SELECT wb.question_id, wb.error_count, wb.last_error_at,
               q.stem, q.topic, q.src, q.difficulty, q.passage, q.chart_data
