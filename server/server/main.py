...
def seed_questions():
    ...
    if count == 0:
        ...
        conn.commit()
        print(f"[seed] 已插入 {count} -> 题目库初始化完成")
    else:
...
def hash_password(password: str) -> str:
    """加盐 SHA-256：存储格式 `salt$hash`，避免相同密码同哈希。"""
    import hmac
...
def check_quiz(data: QuizCheckIn):
    """匿名判分（demo 模式）：给定作答返回每题对错、正确答案与解析，不落库。
...
def add_wrong_question(data: WrongBookIn):
    """添加错题记录，重复错误则计数+1（匿名 demo 模式不落库）"""
    if data.user_id is None:
