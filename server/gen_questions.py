...
cats = Counter(q["cat"] for q in ALL)
types = Counter(q["type"] for q in ALL)
srcs = Counter(q["src"] for q in ALL)
diff = Counter(q["difficulty"] for q in ALL)
print("题库已生成:", os.path.join(data_dir, "questions.json"))
print("总题数:", len(ALL))
print("分类:", dict(cats))
print("题型:", dict(types))
